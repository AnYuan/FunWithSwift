...
Sequences and collections implement methods such as `map(_:)`, `filter(_:)`, and `reduce(_:combine:)` to consume and transform their contents. You can compose these methods together to efficiently implement complex algorithms.
...
The `map(_:)`, `filter(_:)`, and `reduce(_:combine:)` methods compose naturally, so you can collapse the code above into a single operation. The following code example implements the `remainingCost` function, used by the app to display the remaining cost for a recipe.
*/
...
/*:
To sort an array of a type that doesn't have a defined ordering, the `sorted(isOrderedBefore:)` method takes a closure that returns `true` if the first element should be ordered before the second. Let's sort the ingredients by their prices:
*/
