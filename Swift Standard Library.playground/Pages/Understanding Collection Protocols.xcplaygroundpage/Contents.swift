...
- Note: Although we're learning about the `Collection` protocol now, we need to declare conformance in this declaration to the `RandomAccessCollection` protocol, which inherits from `Collection`. You'll read more about `RandomAccessCollection` below.
*/
struct ImageTimeline: RandomAccessCollection {
    ...
    func index(after i: DateIndex) -> DateIndex {
        let nextDay = calendar.date(byAdding: .day, value: 1, to: i.date, options: [])!
    ...
    subscript (i: DateIndex) -> UIImage? {
        ...
        set {
            // `ImageTimeline` expands dynamically to include the earliest and latest dates.
...
extension ImageTimeline: BidirectionalCollection {
    func index(before i: DateIndex) -> DateIndex {
         let previousDay = calendar.date(byAdding: .day, value: -1, to: i.date, options: [])!
...
extension ImageTimeline {
    func distance(from start: DateIndex, to end: DateIndex) -> Int {
        return calendar.components(.day, from: start.date, to: end.date, options: []).day!
    }
    
    func index(_ i: DateIndex, offsetBy n: Int) -> DateIndex {
        let offsetDate = calendar.date(byAdding: .day, value: n, to: i.date, options: [])!
